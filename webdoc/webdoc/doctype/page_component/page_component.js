...
frappe.ui.form.on('Page Component', {
    refresh: function(frm) {
        // Button to generate the component
        frm.add_custom_button(__('Generate Component'), function() {
            ...
            if (!folder_name) {
                frappe.msgprint(__('Error: Folder name could not be determined. Please ensure the appropriate field is filled.'));
                return;
            }

            frappe.call({
                method: 'webdoc.webdoc.block_generator.generate_page_component_template',
                args: {
        ...
        frm.add_custom_button(__('Generate Template Fields'), function() {
            let folder_name = ''; // Initialize folder_name variable
            ...
            frappe.call({
                method: 'webdoc.webdoc.block_generator.generate_frontend_component',
                args: {
                    docname: frm.doc.name,
                    doctype: frm.doc.doctype,
                    folder_name: folder_name // Pass the determined folder name
