    ...
    edit_values(frm, cdt, cdn) {
        ...
        if (isSection) {
            ...
            if (row.section && row.web_template) {
                frappe.msgprint("Error: Both 'Section' and 'Web Template' are filled. Please provide only one.");
                return;
            } else if (!row.section && !row.web_template) {
                frappe.msgprint("Error: Please provide either 'Section' or 'Web Template' when type is Default.");
                return;
            }
        ...
        open_web_template_values_editor(template_name, values, isSection).then((new_values) => {
...
function open_web_template_values_editor(template_name, existing_values, isSection) {
    return new Promise((resolve) => {
        // Fetch the appropriate doctype based on whether it's a Section or Page Block
        const doctype = isSection ? "Web Template" : "Page Component";
                ...
                if (r.message) {
                    ...
                    template.fields.forEach(field => {
                        ...
                        if (field.fieldtype === 'Table Break') {
                            ...
                            if (field.fieldtype === 'Markdown Editor' && field.options === 'Text Editor') {
